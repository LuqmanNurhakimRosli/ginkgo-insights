  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-5 px-5 py-5 lg:grid-cols-[minmax(0,1fr)_380px]">
        <div>
          <MapWorkspace height="h-[520px]" overlay="suitability" showTime={false} />
          <div className="ginkgo-panel mt-4 flex flex-wrap gap-x-6 gap-y-2 px-4 py-3">
            {[
              ...
              <span key={label} className="flex items-center gap-2 text-[12px] text-muted-foreground">
                <span className="h-2.5 w-2.5 rounded-[3px]" style={{ backgroundColor: color }} />
                {label}
              </span>
            ))}
          </div>
          <p className="mt-4 rounded-md border border-warning/40 bg-warning-soft px-4 py-3 text-[12px] leading-snug text-foreground">
        ...
        <aside className="space-y-4">
          ...
          <div className="ginkgo-panel space-y-3 px-4 py-4">
            <h2 className="text-[13.5px] font-semibold">Criteria & Weights</h2>
            {su.criteria.map((c) => (
              <ProgressMetric key={c.label} label={`${c.label} (${c.weightPct}%)`} value={c.score} />
            ))}
          </div>
          <div className="ginkgo-panel px-4 py-4">
            <h2 className="text-[13.5px] font-semibold">Planning Recommendation</h2>
            <p className="mt-2 text-[12.5px] leading-relaxed">{rec.headline}</p>
            ...
            <ol className="mt-3 list-decimal space-y-1 pl-4 text-[12.5px]">
              {rec.actions.map((a) => <li key={a}>{a}</li>)}
            </ol>
            <div className="mt-3 flex flex-wrap gap-2">
              <button onClick={highlight} className="rounded-md bg-primary px-3 py-2 text-[12.5px] font-semibold text-primary-foreground">
                Highlight Areas
              </button>
              <Link to="/reports" className="rounded-md border border-border px-3 py-2 text-[12.5px] font-semibold">
                Generate Report
              </Link>
            </div>
